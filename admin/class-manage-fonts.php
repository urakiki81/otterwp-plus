...
class Otterwp_Fonts_Admin {
    ...
    function create_admin_menu() {
		...
		$manage_fonts_page_title=_x('Manage theme fonts', 'UI String', 'otterwp');
        $manage_fonts_menu_title=_x('Manage theme fonts', 'UI String', 'otterwp');
        ...
        $google_fonts_page_title=_x('Embed Google font in current Theme', 'UI String', 'otterwp');
		$google_fonts_menu_title=_x('Embed Google font in current Theme', 'UI String', 'otterwp');
        add_settings_field(null, $google_fonts_page_title, $google_fonts_menu_title, 'edit_theme_options', 'otterwp-font-settings', [ $this, 'google_fonts_admin_page' ] );

		$local_fonts_page_title=_x('Embed local font in current Theme', 'UI String', 'otterwp');
		$local_fonts_menu_title=_x('Embed local font in current Theme', 'UI String', 'otterwp');
    ...
    function manage_fonts_admin_page () {
        ...
        $asset_file = include locate_template( 'build/index.asset.php' );
        ...
        // Load our app.js.
        wp_enqueue_script( 'create-block-theme-app', get_theme_file_uri( 'build/index.js' ), $asset_file['dependencies'], $asset_file['version'] );

        wp_enqueue_style( 'manage-fonts-styles',  get_theme_file_uri( 'css/manage-fonts.css' ), array(), '1.0', false );
        ...
        if ( ! class_exists( 'WP_Webfonts' ) ) {
            $font_assets_stylesheet = otterwp_render_font_styles($theme_font_families);
    ...
    <div class="wrap">
        ...
        <a href="<?php echo admin_url( 'themes.php?page=add-local-font-to-theme-json' ); ?>" class="page-title-action"><?php _e('Add Local Font', 'otterwp-plus'); ?></a>
        ...
        <form method="POST"  id="manage-fonts-form">
            ...
            <input type="hidden" name="nonce" value="<?php echo wp_create_nonce( 'otterwp_block_theme' ); ?>" />
        </form>
    ...
    function local_fonts_admin_page () {
        wp_enqueue_script('inflate', get_template_directory_uri() . 'js/lib/inflate.js', array( ), '1.0', false );
        wp_enqueue_script('unbrotli', get_template_directory_uri() . 'js/lib/unbrotli.js', array( ), '1.0', false );
        wp_enqueue_script('lib-font-browser', get_template_directory_uri() . 'js/lib/lib-font.browser.js', array( ), '1.0', false );
        wp_enqueue_script('embed-local-font', get_template_directory_uri() . 'js/embed-local-font.js', array( ), '1.0', false );
        ...
        <div class="wrap local-fonts-page">
            <h2><?php _ex('Add local fonts to your theme', 'UI String', 'otterwp-plus'); ?></h2>
            <h3><?php printf( esc_html__('Add local fonts assets and font face definitions to your current active theme (%1$s)', 'otterwp-plus'),  esc_html( wp_get_theme()->get('Name') ) ); ?></h3>
            <form enctype="multipart/form-data" action="" method="POST">
                <table class="form-table">
                    <tbody>
                        <tr>
                            <th scope="row">
                                <label for="font-file"><?php _e('Font file', 'otterwp-plus'); ?></label>
                                <br>
                                <small style="font-weight:normal;"><?php _e('.ttf, .woff, .woff2 file extensions supported', 'otterwp-plus'); ?></small>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th><?php _e('Font face definition for this font file:', 'otterwp-plus'); ?></th>
                            <td>
                                <hr/>
                            </td>
                        </tr>
                        <tr>
                            <th>
                                <label for="font-name"><?php _e('Font name', 'otterwp-plus'); ?></label>
                            </th>
                            <td>
                                <input type="text" name="font-name" id="font-name" placeholder="<?php _e('Font name', 'otterwp-plus'); ?>" required>
                            </td>
                        </tr>
                        <tr>
                            <th>
                                <label for="font-style"><?php _e('Font style', 'otterwp-plus'); ?></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th>
                                <label for="font-weight"><?php _e('Font weight', 'otterwp-plus'); ?></label>
                            </th>
                            <td>
                                <input type="text" name="font-weight" id="font-weight" placeholder="<?php _e('Font weight', 'otterwp-plus'); ?>" required>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<?php
    ...
    function google_fonts_admin_page() {
		wp_enqueue_script('google-fonts-script', get_template_directory_uri() . '/admin/js/google-fonts.js', array( ), '1.0', false );
...
?>
		<div class="wrap google-fonts-page">
			<h2><?php _ex('Add Google fonts to your theme', 'UI String', 'otterwp-plus'); ?></h2>
			<form enctype="multipart/form-data" action="" method="POST">
				<h3><?php printf( esc_html__('Add Google fonts assets and font face definitions to your current active theme (%1$s)', 'otterwp'),  esc_html( wp_get_theme()->get('Name') ) ); ?></h3>
				<label for="google-font-id"><?php printf( esc_html__('Select Font', 'otterwp-plus')); ?></label>
				<select name="google-font" id="google-font-id">
                    <option value=""><?php _e('Select a font...', 'otterwp-plus'); ?></option>
				</select>
				<br /><br />
				<p class="hint"><?php _e('Select the font variants you want to include:', 'otterwp'); ?></p>
				<table class="wp-list-table widefat fixed striped table-view-list" id="google-fonts-table">
					<thead>
						<tr>
							...
							<td class=""><?php printf( esc_html__('Variant', 'otterwp-plus')); ?></td>
							<td class=""><?php printf( esc_html__('Preview', 'otterwp-plus')); ?></td>
						</tr>
					</thead>
				...
				<input type="submit" value="<?php _e('Add google fonts to your theme', 'otterwp'); ?>" class="button button-primary" id="google-fonts-submit" disabled=true />
                <input type="hidden" name="nonce" value="<?php echo wp_create_nonce( 'otterwp_block_theme' ); ?>" />
			</form>
		</div>
	<?php
    ...
    function save_manage_fonts_changes () {
        if (
            ...
            ! empty( $_POST['nonce'] ) &&
            wp_verify_nonce( $_POST['nonce'], 'otterwp_block_theme' ) &&
    ...
    function admin_notice_embed_font_success () {
		...
		?>
			<div class="notice notice-success is-dismissible">
				<p>
                    <?php printf( esc_html__( '%1$s font added to %2$s theme. ', 'otterwp-plus' ), esc_html( $_POST['font-name'] ), esc_html( $theme_name ) ); ?>
                    <a href="themes.php?page=manage-fonts"><?php printf( esc_html__( "Manage Fonts", "otterwp" ) ); ?></a>
                </p>
			</div>
		<?php
	}
	...
	function admin_notice_embed_font_permission_error () {
		...
		?>
			<div class="notice notice-error is-dismissible">
				<p><?php printf( esc_html__( 'Error adding %1$s font to %2$s theme. WordPress lack permissions to write the font assets.', 'otterwp-plus' ), esc_html( $_POST['font-name'] ), esc_html( $theme_name ) ); ?></p>
			</div>
		<?php
	}
    ...
    function admin_notice_embed_font_file_error () {
		...
		?>
			<div class="notice notice-error is-dismissible">
				<p><?php printf( esc_html__( 'Error adding %1$s font to %2$s theme. The uploaded file is not valid.', 'otterwp-plus' ), esc_html( $_POST['font-name'] ), esc_html( $theme_name ) ); ?></p>
			</div>
		<?php
	}
    ...
    function admin_notice_manage_fonts_permission_error () {
		...
		?>
			<div class="notice notice-error is-dismissible">
				<p><?php printf( esc_html__( 'Error handling font changes. WordPress lack permissions to manage the theme font assets.', 'otterwp-plus' ), esc_html( $theme_name ) ); ?></p>
			</div>
		<?php
	}
    ...
    function admin_notice_delete_font_success () {
		...
		?>
			<div class="notice notice-success is-dismissible">
				<p><?php printf( esc_html__( 'Font removed from your theme (%1$s).', 'otterwp_plus' ), esc_html( $theme_name ) ); ?></p>
			</div>
		<?php
	}
}
